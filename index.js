...
const CreateReminderIntentHandler = {
  ...
  async handle(handlerInput) {
    ...
    if(subject && type && date){
    	...
    	for(const [x,y] of Object.entries(TIMETABLE[startDate.getDay()])){
    		if((y.classTitle!='') && (y.classTitle.toLowerCase()==subject.toLowerCase()+' '+type)){
    			time = x;
    			detail = y;
    			break;
    		}
        ...
        alertInfo: {
          spokenInfo: {
            content: [{
              locale: 'en-IN',
              text: 'Hurry! You have '+detail.classTitle+' '+type+' after ten minutes in room no. '+detail.roomNo,
            }],
          },
        },
        pushNotification: {
      ...
      TIMETABLE[startDate.getDay()][time].alertToken=(new String(reminderResponse.alertToken)).toString();
...
const DeleteReminderIntentHandler = {
  ...
  async handle(handlerInput){
    ...
    if(subject && date && type){
    	...
    	try{
    		const client = handlerInput.serviceClientFactory.getReminderManagementServiceClient();
    		let reminderResponse = await client.deleteReminder(TIMETABLE[startDate.getDay()][time].alertToken));
