...
const CreateReminderIntentHandler = {
  ...
  async handle(handlerInput) {
    ...
    if(subject && type && date){
    	...
    	if(detail.alertToken!=""){
    		return handlerInput.responseBuilder
    			.speak("Reminder is already set for "+subject+" "+type+" on "+day[startDate.getDay()])
    			.getResponse();
    	}
      ...
      console.log(reminderResponse.statusCode);
      TIMETABLE[startDate.getDay()].time.alertToken=reminderResponse.alertToken;
      console.log(JSON.stringify(reminderResponse));
    	...
    	return handlerInput.responseBuilder
      	.speak("Reminder for "+subject+" "+type+" on "+day[startDate.getDay()]+" has been created")
      	.getResponse();
    }
...
const DeleteReminderIntentHandler = {
  ...
  async handle(handlerInput){
    ...
    if(subject && date && type){
    	...
    	if(detail.alertToken==""){
    		return handlerInput.responseBuilder
    			.speak("No reminder is set for "+subject+" "+type+" on "+day[startDate.getDay()])
    			.getResponse();
    	}
    	// check for confirmation.  if not confirmed, delegate
    	switch (requestEnvelope.request.intent.confirmationStatus) {
    	...
    	try{
    		...
    		let reminderResponse = await client.deleteReminder(detail.alertToken);
    		if(reminderResponse.statusCode==200){
      		TIMETABLE[startDate.getDay()].time.alertToken="";
    		}
      	console.log(JSON.stringify(reminderResponse));
    	...
    	return handlerInput.responseBuilder
      	.speak("Reminder for "+subject+" "+type+" on "+day[startDate.getDay()]+" has been deleted")
      	.getResponse();
    }
    return handlerInput.responseBuilder
